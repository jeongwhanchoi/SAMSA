...
class LRGBPeptidesFuncExperiment():
    def __init__(self,
                 exp_name,
                 device='cpu'):
        ...
        self.metric = BinaryAveragePrecision(thresholds=5).to(self.device)
    ...
    def run_experiment(self):
        for epoch in range(self.current_epoch, self.number_of_epoch):
            ...
            for i, data in enumerate(loop):
                ...
                outputs = self.model(**inputs_dict)
                outputs = outputs.reshape(outputs.shape[0] * 10, 1)
            ...
            with torch.no_grad():
                for i, data in enumerate(loop):
                    ...
                    outputs = self.model(**inputs_dict)
                    outputs = outputs.reshape(outputs.shape[0] * 10, 1)
                    labels = labels.reshape(labels.shape[0] * 10, 1).to(torch.long)
                    AP = self.metric(F.sigmoid(outputs),labels).detach().cpu().item()
                    val_AP += AP * outputs.shape[0]
                    val_AP_bz += outputs.shape[0]

                val_AP = val_AP / val_AP_bz
            ...
            with torch.no_grad():
                for i, data in enumerate(loop):
                    ...
                    labels = labels.reshape(labels.shape[0] * 10, 1).to(torch.long)
                    AP = self.metric(F.sigmoid(outputs),labels).detach().cpu().item()
                    test_AP += AP * outputs.shape[0]
                    test_AP_bz += outputs.shape[0]

                test_AP = test_AP / test_AP_bz
