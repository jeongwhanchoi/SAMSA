...
class LRARetrievalExperiment():
    def __init__(self,
        ...
        train_dataset_settings = {
            "path_folder": dataset_settings["path_folder"],
            "split": "train"
        }
        ...
        self.n_classes = train_dataset.num_label
        ...
        self.model = nn.ModuleList([
            torch.compile(self.model.to(self.device)), 
            torch.compile(nn.Sequential(
                nn.LazyLinear(1024),
                nn.GELU(),
                nn.LazyLinear(2)
        ).to(self.device))])
        ...
        self.weight = torch.zeros(self.n_classes, device=self.device)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.train_dataloader):
                ...
                labels = F.one_hot(labels.reshape(-1).to(torch.long), num_classes=self.n_classes)
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.val_dataloader):
                ...
                outputs = self.model[0](**inputs_dict)
                outputs1, outputs2 = torch.split(outputs, outputs.shape[0] // 2, 0)
    ...
    def run_experiment(self):
        for epoch in range(self.current_epoch, self.number_of_epoch):
            ...
            whole_dataset_label = []
            correct = torch.zeros(self.n_classes, device=self.device)
            total = torch.zeros(self.n_classes, device=self.device)
            with torch.no_grad():
                for i, data in enumerate(loop):
                    inputs1, inputs2, labels = data
                    inputs1 = inputs1.to(self.device)
                    ...
                    outputs = F.one_hot(torch.argmax(outputs, dim=1), num_classes=self.n_classes)
                ...
                correct = correct[:self.n_classes]
                total = total[:self.n_classes]
