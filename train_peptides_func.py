...
from data.dataset import Peptide_func_dataset
import warnings
...
def train(num_epochs: int,
    ...
    train_dataset = Peptide_func_dataset(root=root,
                                         split='train')

    val_dataset = Peptide_func_dataset(root=root,
                                        split='val')

    test_dataset = Peptide_func_dataset(root=root,
                                        split='test')

    model = parse_graph_architecture(model_str)
    metric = BinaryAveragePrecision(thresholds=5).to('cuda')
    ...
    lr_scheduler      = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=30, gamma=0.8)
    

    max_gradient_norm = 16.0
    global_step       = 0
    start_epoch       = 0
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i in loop:
            ...
            pred = pred.reshape(pred.shape[0] * 10, 1)
            label = label.reshape(label.shape[0] * 10, 1)
            loss = F.binary_cross_entropy_with_logits(pred, label)
            loss = loss / accumulate
            loss.backward()
            label = label.long()
            display_loss = metric(F.sigmoid(pred), label).detach().cpu().item()
        ...
        print("Train Instance AP: " + str(train_loss)[:7] + ". Loss Value: " + str(loss.detach().item())[:7] + ". LR: " + str(round(lr_scheduler.get_last_lr()[0], 8)))

        log_file.write(f"Epoch {epoch+1}\n")
        log_file.write(f"Train Instance AP={train_loss:.5f}, Loss Value={loss.detach().item():.5f}, Lr={lr_scheduler.get_last_lr()[0]:9f}\n")
        ...
        with torch.no_grad():
            ...
            for i in loop:
                node_features, adjacency_matrices, label, mask = val_dataset.get_batch(batch_size=batch_size)
                node_features, adjacency_matrices, label, mask = node_features.to(device), adjacency_matrices.to(device), label.to(device), mask.to(device)
                ...
                label = label.reshape(label.shape[0] * 10, 1)
                label = label.long()
                loss = metric(F.sigmoid(pred), label).detach().cpu().item()
                val_mean_loss.append(loss)
            val_AP = sum(val_mean_loss) / len(val_mean_loss)
            ...
            test_AP = sum(test_mean_loss) / len(test_mean_loss)
            if (val_AP >= best_val_AP):
                ...
                state = {
                    ...
                    'test_instance_acc': test_AP,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                }
                torch.save(state, f"{ckpt_dir}/{model_name}_best.pth")

            print('Val AP: %.5f. Test AP: %.5f'% (val_AP, test_AP))
            print('Best_epoch: %i, Best Val AP: %f, Best Test AP: %f'% (best_epoch, best_val_AP, best_test_AP))

            log_file.write(f"Test Instance AP={test_AP:.5f},\n")
            log_file.write(f"Best Epoch={best_epoch}, Best Instance AP={best_test_AP:.5f}.\n\n")
