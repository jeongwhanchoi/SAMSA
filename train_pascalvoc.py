...
from data.dataset import Pascalvoc_dataset
import warnings
...
def train(num_epochs: int,
    ...
    train_dataset = Pascalvoc_dataset(root=root,
                                         split='train')

    val_dataset = Pascalvoc_dataset(root=root,
                                        split='val')

    test_dataset = Pascalvoc_dataset(root=root,
                                        split='test')

    model = parse_graph_architecture(model_str)
    metric = F1Score(task='multiclass', num_classes=21, average='macro').to('cuda')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i in loop:
            ...
            for j in range(pred.shape[0]):
                preds.append(pred[j,:n_nodes[j],:].reshape(-1, 21))
