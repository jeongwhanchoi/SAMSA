...
def parse_args():
    ...
    parser.add_argument("--input_mask", type=str, help=".")
...
def train(input_mask: bool,
    ...
    model = parse_point_architecture(model_str, is_normal=use_normal, input_mask=input_mask)
    # model.is_ssl = False
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        with torch.no_grad():
            ...
            for points, target in loop:
                points, target = points.cuda(), target.cuda()
                points = points.to(torch.float32)
                points = centralize_pts(points, input_mask)
                if input_mask is True:
...
if __name__ == '__main__':
    ...
    if args.input_mask == 'True':
        args.input_mask = True
    elif args.input_mask == 'False':
