import torch
import torch.nn as nn
import torch.nn.functional as F
from torch import Tensor
from layer.attention.mha import MultiHeadAttention
from layer.norm import RMSNorm, MaskedBatchNorm1d
from functional import get_activation_fn
from typing import List, Union, Any, Callable, Optional


class TransformerEncoderLayer(nn.Module):
    __constants__ = ['norm_first']

    def __init__(self, d_model: int, nhead: int,
                 n_sampled_points, drop_point,
                 score_functions, relative_function, probability_function, norm,
                 dim_feedforward: int = 2048, drop_att: float = 0.1, drop_val: float = 0.1, drop_ffn: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.gelu,
                 layer_norm_eps: float = 1e-5, batch_first: bool = True, norm_first: bool = False,
                 bias: bool = True, device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
        self.self_attn = MultiHeadAttention(d_model, 
                                            nhead, 
                                            n_sampled_points, 
                                            drop_point,
                                            drop_att, 
                                            score_functions, 
                                            relative_function, 
                                            probability_function)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(drop_ffn)
        self.linear2 = nn.Linear(dim_feedforward, d_model, bias=bias, **factory_kwargs)

        self.norm_first = norm_first
        if norm == "LN":
            self.norm1 = nn.LayerNorm(d_model, eps=layer_norm_eps, bias=bias, **factory_kwargs)
            self.norm2 = nn.LayerNorm(d_model, eps=layer_norm_eps, bias=bias, **factory_kwargs)
        elif norm == "RMS":
            self.norm1 = RMSNorm(d_model)
            self.norm2 = RMSNorm(d_model)
        elif norm == "BN":
            self.norm1 = MaskedBatchNorm1d(d_model)
            self.norm2 = MaskedBatchNorm1d(d_model)
        
        self.dropout1 = nn.Dropout(drop_val)
        self.dropout2 = nn.Dropout(drop_val)

        self.res_weight = nn.Parameter(torch.zeros(1), requires_grad=True)
        # self.res_weight = nn.Parameter(torch.ones(1), requires_grad=False)

        # Legacy string support for activation function.
        if isinstance(activation, str):
            activation = get_activation_fn(activation)

        # We can't test self.activation in forward() in TorchScript,
        # so stash some information about it instead.
        if activation is F.relu or isinstance(activation, torch.nn.ReLU):
            self.activation_relu_or_gelu = 1
        elif activation is F.gelu or isinstance(activation, torch.nn.GELU):
            self.activation_relu_or_gelu = 2
        else:
            self.activation_relu_or_gelu = 0
        self.activation = activation
        self.d_model = d_model
        self.norm = norm
        if norm == 'BN':
            self.forward = self.forward_BN

    def __setstate__(self, state):
        super().__setstate__(state)
        if not hasattr(self, 'activation'):
            self.activation = F.relu

    def forward_BN(
            self,
            src: Tensor,
            position,
            relative_map,
            mask=None) -> Tensor:

        x = src
        if mask is None:
            mask = torch.zeros(src.shape[0], src.shape[1], device=src.device)
        if self.norm_first:
            x_ = self._sa_block(self.norm1(x, input_mask=mask), position, relative_map, mask)
            x = (x[:,:x_.shape[1],:] + x_) * (1 - mask).unsqueeze(-1)
            x = (x + self._ff_block(self.norm2(x, input_mask=mask))) * (1 - mask).unsqueeze(-1)
        else:
            x_ = self._sa_block(x, position, relative_map, mask)
            x = self.norm1(x[:,:x_.shape[1],:] + x_, input_mask=mask) * (1 - mask).unsqueeze(-1)
            x = self.norm2(x + self._ff_block(x), input_mask=mask) * (1 - mask).unsqueeze(-1)
        return x

    def forward(
            self,
            src: Tensor,
            position,
            relative_map,
            mask=None) -> Tensor:
        if mask is None:
            mask = torch.zeros(src.shape[0], src.shape[1], device=src.device)
        x = src * (1 - mask).unsqueeze(-1)
        
        if self.norm_first:
            x_ = self._sa_block(self.norm1(x), position, relative_map, mask)
            x = (x[:,:x_.shape[1],:] + x_) * (1 - mask).unsqueeze(-1)
            x = (x + self._ff_block(self.norm2(x))) * (1 - mask).unsqueeze(-1)
        else:
            x_ = self._sa_block(x, position, relative_map, mask)
            x = self.norm1(x[:,:x_.shape[1],:] + x_) * (1 - mask).unsqueeze(-1)
            x = self.norm2(x + self._ff_block(x)) * (1 - mask).unsqueeze(-1)
        return x

    # self-attention block
    def _sa_block(self, x: Tensor, position, relative_map, mask=None) -> Tensor:
        x, _ = self.self_attn(x, x, x, position, relative_map, mask)
        x = x * self.res_weight
        return self.dropout1(x)

    # feed forward block
    def _ff_block(self, x: Tensor) -> Tensor:
        x = self.linear2(self.dropout(self.activation(self.linear1(x)))) * self.res_weight
        return self.dropout2(x)