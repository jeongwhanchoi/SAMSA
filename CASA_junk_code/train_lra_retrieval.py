...
import argparse
import logging
from model import parse_lra_architecture
...
def train(num_epochs: int,
    ...
    train_dataset = RetrievalDataset(split='train', path_folder=root)
    test_dataset = RetrievalDataset(split='test', path_folder=root)
    ...
    model = nn.ModuleList([parse_lra_architecture(model_str),
                           nn.LazyLinear(2)])
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for input1, input2, target in loop:
            ...
            target_oh = F.one_hot(target.long(), num_classes=2).to(input1.dtype)
            # pred1 = model(input1)
            # pred2 = model(input2)
            pred1 = torch.max(checkpoint_sequential(model[0], 4, input1), dim=1)[0]
            pred2 = torch.max(checkpoint_sequential(model[0], 4, input2), dim=1)[0]
            pred = model[1](torch.cat([pred1, pred2], dim=-1))
        ...
        with torch.no_grad():
            ...
            for input1, input2, target in loop:
                ...
                pred2 = torch.max(checkpoint_sequential(model[0], 4, input2), dim=1)[0]
                pred = model[1](torch.cat([pred1, pred2], dim=-1))
                # pred1 = checkpoint_sequential(model, 4, input1)
                # pred2 = checkpoint_sequential(model, 4, input2)
