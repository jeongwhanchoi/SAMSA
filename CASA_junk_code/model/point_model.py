...
from layer import RESC, RSTL, RandomColor, SoftplusPWP, SAM, GlobalNode, SSCL, RSTLBlock,RSTLRBlock
...
def create_layer(layer_type, hyperparams):
    ...
    elif layer_type == "RSTLB":
        return RSTLBlock(*hyperparams)
    elif layer_type == "RSTLRB":
        return RSTLRBlock(*hyperparams)
...
class PointModel(nn.Module):
    def __init__(self, layer_list, is_normal=False, is_rotation=True):
        ...
        for i in range(len(self.layer_list)):
            if isinstance(self.layer_list[i], RSTL) or isinstance(self.layer_list[i], RSTLBlock) or isinstance(self.layer_list[i], RSTLRBlock):
        ...
        if self.is_rotation is True:
            ...
            self.normalization = nn.BatchNorm1d(3, affine=False)

    def forward_notrelative(self, x, mask=None):
        x[:,:,:3] = self.normalization(x[:,:,:3].reshape(-1, 3)).reshape(x.shape[0], x.shape[1], 3)
        ...
        for i in range(len(self.layer_list)):
            ...
            elif isinstance(self.layer_list[i], RESC):
    ...
    def forward_relative(self, x, mask=None): # relative
        ...
        if self.is_normal == False:
            x_ref = x[:, :, :3] * 1
            # centroid = torch.mean(x_ref[:,:,:3], dim=1, keepdim=True)
            # max_distance = torch.max(torch.sqrt(torch.sum((x_ref[:,:,:3] - centroid) ** 2, dim=2, keepdim=True)), dim=1, keepdim=True)[0]
...
class PointModelMTL(nn.Module):
    ...
    def append_task(self, x):
        self.task_heads.append(x)
    ...
    def forward_notrelative(self, x, mask=None):
        for i in range(len(self.layer_list)):
            if isinstance(self.layer_list[i], nn.LazyLinear) or isinstance(self.layer_list[i], nn.Linear):
                ...
                x = self.layer_list[i](x, mask)
        return self.task_heads[self.task](x)
...
def parse_point_architecture_mtl(arch_string, is_normal=False, is_rotation=True):
    ...
    return PointModelMTL(sequential_layer, is_normal, is_rotation)
