...
class SAMAttention(nn.Module):
    ...
    def forward_noleaky(self, q, k, v, c, mask=None, e=1e-12):
        # input is 4 dimension tensor
        # [batch_size, head, length, d_tensor]
        if mask is None:
            scores = F.relu(self.score_functions(q, k))
            detr = torch.sum(scores, dim=-1, keepdim=True)
            # detr = detr + torch.sign(detr) * 1e-9
            scores = scores / (detr + 1e-9)
        else:
    ...
    def forward_leaky(self, q, k, v, c, mask=None, e=1e-12):
        # input is 4 dimension tensor
        # [batch_size, head, length, d_tensor]
        if mask is None:
            scores = F.relu(self.score_functions(q, k))
            detr = torch.sum(scores, dim=-1, keepdim=True) + F.softplus(torch.sum(c, dim=2, keepdim=True)) + 1e-9
            scores = scores / detr
        else:
            mask = mask.unsqueeze(1) # Input mask = (batch, n_seq, 1)
            mask = mask.unsqueeze(1)
...
class SAMCMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        out, attention = self.attention(q, k, v, c, mask=mask)
...
class SAMMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        kv, regularization = self.SAM(k, v, mask=mask)
...
class EuclideanSAMMultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        out, attention, regularization = self.att(q, k, v, mask)
        return out, attention, regularization
