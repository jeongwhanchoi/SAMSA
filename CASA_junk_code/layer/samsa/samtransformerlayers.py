...
class CustomSAMTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int,
        ...
        super().__init__()
        self.self_attn = attention_type(d_model, nhead, n_sampled_points_lb, n_sampled_points_ub, drop_point, leaky_softmax=leaky_softmax)
    ...
    def forward(
        ...
        x = src
        if self.norm_first:
            sa, regularization = self._sa_block(self.norm1(x), mask)
            x = x + sa
            x = x + self._ff_block(self.norm2(x))
        else:
            sa, regularization = self._sa_block(x, mask)
            x = self.norm1(x + sa)
    ...
    def _sa_block(self, x: Tensor, mask=None) -> Tensor:
        x, _, regularization = self.self_attn(x, x, x, mask)
        x = x * self.res_weight
