...
from layer.sscl.sscl_mha import SSCL_ScaledDotSAMMultiHeadAttention, SSCL_EuclideanSAMMultiHeadAttention, SSCL_CompositeSAMMultiHeadAttention
...
class SSCL_CustomSAMTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int,
                 ...
                 bias: bool = True, device=None, dtype=None, attention_type=SSCL_ScaledDotSAMMultiHeadAttention,
    ...
    def _sa_block(self, x: Tensor, position, mask=None) -> Tensor:
        x = self.self_attn(x, x, x, position, mask)[0] * self.res_weight
...
class SSCL_ScaledDotSAMTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model: int, nhead: int,
                 n_sampled_points_lb, n_sampled_points_ub, drop_point, drop_global_points,
                 dim_feedforward: int = 2048, dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.gelu,
                 ...
                 normtype="LayerNorm") -> None:
        super().__init__()
        self.layer = SSCL_CustomSAMTransformerEncoderLayer(d_model, nhead, 
                                                      n_sampled_points_lb, n_sampled_points_ub, drop_point, drop_global_points,
                                                      dim_feedforward, dropout,
                                                      activation,
                                                      layer_norm_eps, batch_first, norm_first,
                                                      bias, device, dtype, SSCL_ScaledDotSAMMultiHeadAttention, normtype)
...
class SSCL_EuclideanSAMTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model: int, nhead: int,
                 n_sampled_points_lb, n_sampled_points_ub, drop_point, drop_global_points,
                 dim_feedforward: int = 2048, dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.gelu,
        ...
        self.layer = SSCL_CustomSAMTransformerEncoderLayer(d_model, nhead, 
                                                      ...
                                                      activation,
                                                      layer_norm_eps, batch_first, norm_first,
                                                      bias, device, dtype, SSCL_EuclideanSAMMultiHeadAttention, normtype)
...
class SSCL_CompositeSAMTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model: int, nhead: int,
                 n_sampled_points_lb, n_sampled_points_ub, drop_point, drop_global_points,
                 dim_feedforward: int = 2048, dropout: float = 0.1,
                 activation: Union[str, Callable[[Tensor], Tensor]] = F.gelu,
        ...
        self.layer = SSCL_CustomSAMTransformerEncoderLayer(d_model, nhead, 
                                                      ...
                                                      activation,
                                                      layer_norm_eps, batch_first, norm_first,
                                                      bias, device, dtype, SSCL_CompositeSAMMultiHeadAttention, normtype)
