...
def pairwise_sort_sample(x, x_score, relative_map, k): 
    ...
    x_bottom = torch.cat([x_bottom, torch.ones_like(x)], dim=1)
    x_score_bottom = torch.cat([x_score_bottom, torch.ones_like(x_score) * -1e8], dim=1)
    ...
    if relative_map is not None:
        ...
        return x, x_top.squeeze(2), x_bottom.squeeze(2), x_score, None, None